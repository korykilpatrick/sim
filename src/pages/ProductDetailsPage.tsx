...
const ProductDetailsPage: React.FC = () => {
  ...
  const handleBuyNow = () => {
    ...
    if (
      ...
      if (isAuthenticated) {
        ...
        navigate('/auth/login', { state: { from: `/protected/configure/${product.id}` } });
      }
      return;
    }
  ...
  const maritimeAlertProduct = isMaritimeAlert ? (product as MaritimeAlertProduct) : null;
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          <ProductImageGallery mainImage={product.imageUrl} alt={product.name} />
        </div>
              ...
              {isMaritimeAlert && maritimeAlertProduct && (
                <div className="mt-6">
                  ...
                  <ul className="list-disc list-inside space-y-2 text-secondary-700">
                    {maritimeAlertProduct.alertTypesAvailable.map(alertType => (
                      <li key={alertType}>
                        {alertType === 'SHIP' && 'Ship-based alerts - Track specific vessels'}
                        {alertType === 'AREA' && 'Area-based alerts - Monitor defined geographical areas'}
                        {alertType === 'SHIP_AND_AREA' && 'Combined ship and area monitoring'}
                      </li>
                    ))}
                  </ul>
                </div>
