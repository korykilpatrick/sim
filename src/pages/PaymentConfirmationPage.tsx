...
const PaymentConfirmationPage: React.FC = () => {
  ...
  const launchableProducts = items.filter(
    item => ['VTS', 'AMS', 'FTS', 'MARITIME_ALERT'].includes(item.product.type)
  );
  
  const hasReports = items.some(
    item => ['REPORT_COMPLIANCE', 'REPORT_CHRONOLOGY'].includes(item.product.type)
  );
    ...
    <div className="max-w-2xl mx-auto">
      <div className="bg-white rounded-lg shadow p-8 text-center">
        ...
        <div className="space-y-6 mb-8">
          <div>
            <h3 className="text-lg font-medium text-secondary-900 mb-3">Order Summary</h3>
              ...
              {items.map((item, index) => (
                ...
                >
                  <div className="text-left">
                    <p className="text-secondary-900 font-medium">{item.product.name}</p>
        ...
        <div className="space-y-3">
          ...
          <Link to="/marketplace">
            <Button variant={!launchableProducts.length && !hasReports ? "primary" : "outline"} fullWidth>
              Continue Shopping
            </Button>
          </Link>
        </div>
