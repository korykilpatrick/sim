...
const initialCart = persistedCart ? JSON.parse(persistedCart) : { items: [], totalAmount: 0, totalCredits: 0 };
    ...
    addItem: (state, action: PayloadAction<CartItem>) => {
      ...
      const existingItem = state.items.find(item => 
        ...
        (item.product.id === newItem.product.id && 
          JSON.stringify(item.configurationDetails) === JSON.stringify(newItem.configurationDetails))
      );
